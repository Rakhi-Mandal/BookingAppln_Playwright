...
exports.TestimPage = class TestimPage {
  constructor(page) {
    ...
    this.pageLogo=page.locator("//div[contains(@class,'page-header')]//child::img[contains(@alt,'Testim Logo')]")
    this.navigationElements=page.locator("//div[contains(@class,'page-header')]//child::li[contains(@class,'has-drop')]");
    ...
    this.footerLinks =  page.locator("//div[@class='f-bottom']//child::div//ul[@class='l-list']//li//a");
  ...
  async getNavOptions() {
    let headerOptions;
    await helper.assertAllureStep('Get all the available header options', async () => {
  ...
  async validateTheHeaderOptions(){ 
    await helper.assertAllureStep('validating available header options', async () => {
  ...
  async validateCompanySection(){
    await helper.assertAllureStep('validating company section options', async () => {
    await this.companyOption.click();
    for(const option of data.subSectionTexts)
    {
      const subSectionText= this.subSection(option)
      await expect(subSectionText,"validate about sub-section visibility").toBeVisible(parseInt(process.env.smallTimeOut));
    }
  });
...
async validateCustomerSubSection(){
  await helper.assertAllureStep('Fetching and validating company sub-section options', async () => {
    for (const option of data.subSectionTexts) {
      if(option.includes('Customers'))
      {
        const customersSubOption= this.subSection(option)
        helper.logToFile(`Customer sub section options: ${customersSubOption}`)
  ...
  await expect(this.page,"validate the url when we click on testim customer sub-section").toHaveURL(process.env.testimCustomerUrl);
});
  await helper.assertAllureStep('validating customer name', async () => {
  await expect(this.customerName,"validate the customer name in the review section").toHaveText(data.customerSubSection.name);
});
await helper.assertAllureStep('double validating customer details in review section with the data file', async () => {
  ...
  expect(cleanedReceivedContent,"comparing the customer review contents stored in the data file with the reveived one from UI").toBe(reviewContent);
  expect(displayedPosition,"comparing the customer review position stored in the data file with the reveived one from UI").toBe(reviewPosition);
});

}
async getFooterOptions() {
  let footerOptions;
  await helper.assertAllureStep('get all options available in footer', async () => {
    footerOptions = this.footerLinks.elementHandles();
});
return await footerOptions;
}
async validateFooterSection(){
  await helper.assertAllureStep('validating all the footer options are visible available as links', async () => {
  await this.footer.scrollIntoViewIfNeeded();
  expect(this.footer,"validate the footer is not hidden").not.toBeHidden();
 const footOptions = await this.getFooterOptions();
    ...
    for (const option of footOptions) {
      ...
      expect(hrefValue,"validate the options we have in page footer have an attribute 'href'" ).not.toBeNull(); 
